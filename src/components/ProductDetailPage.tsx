...
interface Props {
  ...
  onBack: () => void;
  onBook: (product: AdminProduct, method?: 'razorpay' | 'whatsapp') => void;
}
...
export const ProductDetailPage: React.FC<Props> = ({ product, onBack, onBook }) => {
  ...
  const addonWaUrl = (a: { name: string; price: number }) =>
    `https://wa.me/917022058460?text=${encodeURIComponent(
      `*${product.name}*\nCategory: ${product.categoryName}\nPrice: Rs.${product.price.toLocaleString()}\n\nI'd like to add: *${a.name}* (+Rs.${a.price})\n\nPlease confirm availability.`
    )}`;
      ...
      <div className="mt-10 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {product.addOns.map((a, i) => (
                <div key={i} className="rounded-card border border-border bg-white p-4">
                  ...
                  <div className="mt-1 text-sm font-bold text-brand-purple">+Rs.{a.price.toLocaleString()}</div>
                  <WhatsAppButton href={addonWaUrl(a)} className="mt-3 w-full py-2 text-xs" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col items-start justify-between gap-4 rounded-card bg-brand-purple/5 p-6 sm:flex-row sm:items-center">
          ...
          <div className="flex w-full flex-shrink-0 gap-3 sm:w-auto">
            <BookButton onClick={() => onBook(product, 'razorpay')} className="flex-1 sm:flex-none" />
            <WhatsAppButton onClick={() => onBook(product, 'whatsapp')} className="flex-1 sm:flex-none" />
          </div>
        </div>
